...
function ItemContent({ className, ...props }: React.ComponentProps<'div'>) {
    return <div data-slot="item-content" className={cn('flex flex-1 min-w-0 flex-col gap-1 [&+[data-slot=item-content]]:flex-none', className)} {...props} />;
}

function ItemTitle({ className, ...props }: React.ComponentProps<'div'>) {
    return <div data-slot="item-title" className={cn('flex w-full items-center gap-2 text-sm leading-snug font-medium break-all', className)} {...props} />;
}
