...
@injectable()
export class ClaudeAiProvider implements IAiGenerationService {
    public constructor(@inject(DI_CONSTANTS.ISettingsService) private readonly settingsService: ISettingsService) {}

    public isAvailable(): boolean {
        return this.settingsService.getSettings().claudeApiKey.length > 0;
    }

    public async generateCategory(request: IGenerateCategoryRequest): Promise<IGenerateCategoryResponse> {
        const apiKey = this.settingsService.getSettings().claudeApiKey;
        const content = await callAnthropic(CONFIG.ANTHROPIC_API_URL, CONFIG.ANTHROPIC_MODEL, apiKey, buildCategoryMessages(request), 'Claude');
